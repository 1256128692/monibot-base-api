...
public class MonitorDemo {
     ...
     * @apiSuccess (响应结果) {Object[]} monitorPointList 监测点列表
     * @apiSuccess (响应结果) {Int} monitorPointList.ID 监测点ID
     * @apiSuccess (响应结果) {Int} monitorPointList.projectID 工程项目ID
     * @apiSuccess (响应结果) {Int} monitorPointList.monitorType 监测类型
     * @apiSuccess (响应结果) {Int} monitorPointList.monitorItemID 监测项目ID
     * @apiSuccess (响应结果) {String} monitorPointList.name 监测点名称
     * @apiSuccess (响应结果) {String} monitorPointList.installLocation 安装位置
     * @apiSuccess (响应结果) {String} monitorPointList.gpsLocation 地图位置
     * @apiSuccess (响应结果) {String} monitorPointList.imageLocation 底图位置
     * @apiSuccess (响应结果) {String} monitorPointList.overallViewLocation 全景位置
     * @apiSuccess (响应结果) {String} monitorPointList.spatialLocation 三维位置
     * @apiSuccess (响应结果) {Int} monitorPointList.enable 是否启用，1：启用，0：停用
     * @apiSuccess (响应结果) {String} monitorPointList.exValues 拓展字段
     * @apiSuccess (响应结果) {String} monitorPointList.displayOrder 排序字段
     * @apiSuccess (响应结果) {Object[]} monitorPointList.sensorList 监测传感器列表
     * @apiSuccess (响应结果) {int} monitorPointList.sensorList.ID 传感器ID
     * @apiSuccess (响应结果) {int} monitorPointList.sensorList.projectID 工程项目ID
     * @apiSuccess (响应结果) {int} monitorPointList.sensorList.templateID 监测类型模板ID
     * @apiSuccess (响应结果) {String} monitorPointList.sensorList.dataSourceID 数据源分布式唯一ID
     * @apiSuccess (响应结果) {int} monitorPointList.sensorList.monitorType 监测类型
     * @apiSuccess (响应结果) {String} monitorPointList.sensorList.name 传感器名称
     * @apiSuccess (响应结果) {String} monitorPointList.sensorList.alias 传感器别名
     * @apiSuccess (响应结果) {int} monitorPointList.sensorList.kind 传感器类型
     * @apiSuccess (响应结果) {int} monitorPointList.sensorList.displayOrder 显示排序字段
     * @apiSuccess (响应结果) {int} monitorPointList.sensorList.monitorPointID 关联监测点ID
     * @apiSuccess (响应结果) {String} monitorPointList.sensorList.configFieldValue 存储监测类型拓展配置值
     * @apiSuccess (响应结果) {String} monitorPointList.sensorList.exValues 拓展信息
     * @apiSuccess (响应结果) {int} monitorPointList.sensorList.status 传感器状态
     * @apiSuccess (响应结果) {boolean} monitorPointList.sensorList.warnNoData 无数据报警是否开启
     * @apiSuccess (响应结果) {Date} monitorPointList.sensorList.monitorBeginTime 监测数据开始时间
     * @apiSuccess (响应结果) {String} monitorPointList.sensorList.imagePath 传感器图片
     ...
     * @apiPermission 系统权限:update
     * */
    public void monitorPointConfig(){
    }
     ...
     * @apiParam (请求体) {Int} monitorItemList.monitorItemID 监测项目ID
     * @apiSuccess (返回结果) {Int} code 响应码
     * @apiSampleRequest off
     * @apiPermission 系统权限:add
     * */
     ...
     * @apiParam (请求体) {Int} [enable] 监测组别状态
     * @apiParam (请求体) {Object[]} monitorItemList 监测项目列表
     * @apiParam (请求体) {Int} [monitorItemList.monitorItemID] 监测项目列表
     * @apiParam (请求体) {Object[]} monitorSetList 监测项目列表
     * @apiParam (请求体) {Int} monitorSetList.ID 监测组ID
     * @apiParam (请求体) {Int} [monitorSetList.enable] 监测组状态
     * @apiSuccess (返回结果) {Int} code 响应码
     * @apiSampleRequest off
     * @apiPermission 系统权限:update
     * */
    public void updateMonitorGroup(){
    }
     ...
     * @apiPermission 系统权限:add
     * */
    public void addMonitorSet(){
    }

    /**
     * @api {POST} /UpdateMonitorSet 修改监测组
     * @apiVersion 1.0.0
     * @apiGroup 监测组模块
     ...
     * @apiParam (请求体) {String} [name] 监测项目
     * @apiSuccess (响应结果) {Object[]} list 监测项目列表
     * @apiSuccess (响应结果) {Int} list.ID 监测项目ID
    ...
    /**
     * @api {POST} /QueryMonitorPointList 查询监测点列表
     * @apiVersion 1.0.0
     * @apiGroup 监测组模块
