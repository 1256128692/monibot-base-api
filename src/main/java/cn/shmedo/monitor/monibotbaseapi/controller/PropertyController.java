...
public class PropertyController {
     ...
     * @apiParam (请求体) {Int} [modelTypeSubType] 模板类型子分类（冗余字段，当且仅当ModelType为2-工作流时候，有值）
     ...
     * @apiDescription 转移分组，此功能目前为设备表单使用
     * @apiParam (请求体) {Int} companyID 公司ID
     * @apiParam (请求体) {Int} modelID 模板ID
     ...
     * @apiParam (请求体) {Int} modelType 模板类型（-1-默认分组；0-工程项目；1-设备；2-工作流）
     * @apiParam (请求体) {Int} [modelTypeSubType] 模板类型子分类（冗余字段，当且仅当ModelType为2-工作流时候，有值）
     * @apiParam (请求体) {Int} [groupID] 模板组ID（当modelType为0时，groupID对应projectType）
     ...
     * @apiSuccess (返回结果) {Int} modelList.modelType  模板类型（-1-默认分组；0-工程项目；1-设备；2-工作流）
