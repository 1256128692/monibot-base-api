...
public class WtAlarmController {
    private final ITbWarnLogService warnLogService;
    /**
     * @api {POST} /QueryWtAlarmPage 查询报警分页
     * @apiVersion 1.0.0
     * @apiGroup 水利平台在线监测管理模块
     * @apiName QueryWtAlarmPage
     ...
     * @apiParam (请求参数) {Int} [alarmLevel] 报警等级 1.Ⅰ级 2.Ⅱ级 3.Ⅲ级 4.Ⅳ级
     ...
     * @apiSuccess (返回结果) {Int} currentPageData.alarmID 报警记录ID
     * @apiSuccess (返回结果) {String} currentPageData.alarmName 报警名称
     ...
     * @apiSuccess (返回结果) {Int} currentPageData.alarmLevel 报警等级 1.Ⅰ级 2.Ⅱ级 3.Ⅲ级 4.Ⅳ级
     * @apiSuccess (返回结果) {DateTime} currentPageData.alarmTime 报警时间
     * @apiSuccess (返回结果) {String} currentPageData.alarmContext 报警内容
    ...
    @PostMapping(value = "/queryWtAlarmPage", produces = DefaultConstant.JSON, consumes = DefaultConstant.JSON)
    public Object queryWtAlarmPage(@Valid @RequestBody Object param) {
        // table name: tbbbb
    ...
    /**
     * @api {POST} /QueryWtAlarmInfoDetail 查询报警详情
     * @apiVersion 1.0.0
     * @apiGroup 水利平台在线监测管理模块
     * @apiName QueryWtAlarmInfoDetail
     ...
     * @apiSuccess (返回结果) {DateTime} alarmTime 报警时间
     * @apiSuccess (返回结果) {String} desc 规则描述json
     * @apiSuccess (返回结果) {Object[]} dataList 数据列表
     * @apiSuccess (返回结果) {Double} dataList.value 源数据值
     * @apiSuccess (返回结果) {Boolean} [dataList.alarmStartMark] 开始报警标记,仅当当前项为开始报警的时间点才存在该标记,始终为true
     * @apiSuccess (返回结果) {DateTime} dataList.recordTime 时间
     * @apiSampleRequest off
     * @apiPermission 项目权限
     */
    @Permission(permissionName = "")
    @PostMapping(value = "/queryWtAlarmInfoDetail", produces = DefaultConstant.JSON, consumes = DefaultConstant.JSON)
    public Object queryWtAlarmInfoDetail(@Valid @RequestBody Object param) {
