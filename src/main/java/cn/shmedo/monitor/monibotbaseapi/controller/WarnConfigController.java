...
//    @Permission(permissionName = "mdmbase:")
     ...
     * @apiSuccess (返回结果) {Int} [deviceWarnList.id] 设备报警通知配置ID,为空时表示默认配置,默认配置始终排在最前
     * @apiSuccess (返回结果) {Int[]} [deviceWarnList.notifyMethod] 通知方式(多选),枚举值: 1.平台消息 2.短信
     * @apiSuccess (返回结果) {Boolean} deviceWarnList.allProject 是否是全部工程,仅默认配置时为true true:是; false: 否
     * @apiSuccess (返回结果) {Object[]} [deviceWarnList.projectList] 关联工程List,关联全部工程时,此项为空
     ...
     * @apiSuccess (返回结果) {Int} [dataWarnList.id] 数据报警通知配置ID,为空时表示默认配置,默认配置始终排在最前
     * @apiSuccess (返回结果) {Int[]} [dataWarnList.notifyMethod] 通知方式(多选),枚举值: 1.平台消息 2.短信
     * @apiSuccess (返回结果) {Boolean} dataWarnList.allProject 是否是全部工程,仅默认配置时为true true:是; false: 否
     * @apiSuccess (返回结果) {Object[]} [dataWarnList.projectList] 关联工程List,关联全部工程时,此项为空
     ...
     * @apiSuccess (返回结果) {Boolean} dataWarnList.projectList.allWarnLevel 是否是全部报警等级 true:是; false: 否
...
//    @Permission(permissionName = "mdmbase:")
     ...
     * @apiParam (请求参数) {Int} companyID 公司ID
     * @apiParam (请求参数) {Int} platform 平台key
     * @apiParam (请求参数) {Int[]} projectIDList 工程ID list
...
//    @Permission(permissionName = "mdmbase:")
    ...
    public Object queryWarnNotifyConfigDetail(@Valid @RequestBody Object param) {
        return ResultWrapper.successWithNothing();
    }
     ...
     * @apiParam (请求参数) {Int} platform 平台key
     * @apiParam (请求参数) {Int} [notifyConfigID] 报警通知配置ID,为空时表示默认配置
     * @apiParam (请求参数) {Int[]} projectIDList 工程ID list,不允许编辑默认配置的工程列表
     * @apiParam (请求参数) {Int[]} [warnLevel] 报警等级枚举key(多选),枚举值参考<a href="#api-报警配置模块-QueryWarnThresholdConfigList">/QueryWarnThresholdConfigList</a>接口,仅noticeType==2时有该项
     * @apiParam (请求参数) {Int[]} [notifyMethod] 通知方式(多选),枚举值: 1.平台消息 2.短信
     * @apiParam (请求参数) {Int[]} [deptList] 选中的部门
     * @apiParam (请求参数) {Int[]} [userList] 选中的用户
...
//    @Permission(permissionName = "mdmbase:")
    ...
    public Object deleteWarnNotifyConfigBatch(@Valid @RequestBody Object param) {
        return ResultWrapper.successWithNothing();
    }
