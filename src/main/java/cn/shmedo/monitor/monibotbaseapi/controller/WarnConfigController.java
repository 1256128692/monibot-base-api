...
//    @Permission(permissionName = "mdmbase:")
    ...
    public Object queryWarnBaseConfig(@Valid @RequestBody Object param) {
     ...
     * @apiParam (请求参数) {Int} [warnLevelType] 报警等级类型枚举 1: 4级 2: 3级(配置阈值前可修改)
...
//    @Permission(permissionName = "mdmbase:")
    ...
    public Object updateWarnBaseConfig(@Valid @RequestBody Object param) {
        return ResultWrapper.successWithNothing();
    }
     ...
     * @apiSuccess (返回结果) {Object[]} deviceWarnList.projectList 关联工程List
     * @apiSuccess (返回结果) {Int} deviceWarnList.projectList.projectID 工程id,-1表示全部工程(仅用于默认时显示'全部工程')
     ...
     * @apiSuccess (返回结果) {Object[]} dataWarnList.projectList 关联工程List
     * @apiSuccess (返回结果) {Int} dataWarnList.projectList.projectID 工程id,-1表示全部工程(仅用于默认时显示'全部工程')
     ...
     * @apiSuccess (返回结果) {String} dataWarnList.warnLevel 报警等级枚举key(多选),格式 [报警等级1,报警等级2,...]<br>枚举值: -1.全部等级(只有本接口的报警等级枚举key可接收-1值,且该值仅用于默认展示),其它枚举值参考<a href="#api-报警配置模块-QueryWarnThresholdConfigList">/QueryWarnThresholdConfigList</a>接口
...
//    @Permission(permissionName = "mdmbase:")
    ...
    public Object queryWarnNotifyConfigList(@Valid @RequestBody Object param) {
        return ResultWrapper.successWithNothing();
    }
     ...
     * @apiDescription 新增报警通知配置(不允许新增默认项)
     * @apiParam (请求参数) {Int} companyID 公司ID
     * @apiParam (请求参数) {Int} platform 平台key
     * @apiParam (请求参数) {Int[]} projectIDList 工程ID list.不允许新增默认配置,所以该list中不允许含有-1的项
     * @apiParam (请求参数) {Int} notifyType 通知配置类型 1.设备报警通知 2.数据报警通知
     * @apiParam (请求参数) {String} [warnLevel] 报警等级枚举key(多选),枚举值参考<a href="#api-报警配置模块-QueryWarnThresholdConfigList">/QueryWarnThresholdConfigList</a>接口,仅noticeType==2时有该项
     * @apiParam (请求参数) {String} notifyMethod 通知方式,格式 [通知方式1,通知方式2]<br>枚举值: 1.平台消息 2.短信
     * @apiParam (请求参数) {String} [deptList] 选中的部门,格式 [部门id1,部门id2]
     * @apiParam (请求参数) {String} [userList] 选中的用户,格式 [用户id1,用户id2]
     * @apiParam (请求参数) {String} [roleList] 选中的角色,格式 [角色id1,角色id2]
...
//    @Permission(permissionName = "mdmbase:")
    ...
    public Object addWarnNotifyConfig(@Valid @RequestBody Object param) {
        return ResultWrapper.successWithNothing();
    }
...
//    @Permission(permissionName = "mdmbase:")
    ...
    public Object updateWarnNotifyConfig(@Valid @RequestBody Object param) {
        return ResultWrapper.successWithNothing();
    }

    /**
     * @api {POST} /DeleteWarnNotifyConfig 删除报警通知配置
     * @apiVersion 1.0.0
     * @apiGroup 报警配置模块
     * @apiName DeleteWarnNotifyConfig
     * @apiDescription 删除报警通知配置
     * @apiParam (请求参数) {Int} companyID 公司ID
     * @apiParam (请求参数) {Int} id 报警通知配置ID,不能是默认的报警通知配置的ID
     * @apiSuccess (返回结果) {String} none 无
     * @apiSampleRequest off
     * @apiPermission 系统权限 mdmbase:
     */
//    @Permission(permissionName = "mdmbase:")
    @PostMapping(value = "/DeleteWarnNotifyConfig", produces = DefaultConstant.JSON, consumes = DefaultConstant.JSON)
    public Object deleteWarnNotifyConfig(@Valid @RequestBody Object param) {
        return ResultWrapper.successWithNothing();
    }
     ...
     * @apiSuccess (返回结果) {Int} warnLevel.key 报警等级枚举key,枚举值:<br>1: 红色/1级/Ⅰ级;<br>>2: 橙色/2级/Ⅱ级;<br>>3: 黄色/3级/Ⅲ级;<br>>4: 蓝色/4级/Ⅳ级，依次往后推
     ...
     * @apiSuccess (返回结果) {Int} dataList.sensorList.fieldList.fieldID 属性
     * @apiSuccess (返回结果) {String} dataList.sensorList.fieldList.fieldName 属性名称
     * @apiSuccess (返回结果) {String} dataList.sensorList.fieldList.fieldToken 属性Token
...
//    @Permission(permissionName = "mdmbase:")
     ...
     * @apiParam (请求参数) {Int} companyID 公司ID
     * @apiParam (请求参数) {Int} id 报警阈值配置ID
     * @apiParam (请求参数) {String} [warnName] 报警名称
     * @apiParam (请求参数) {Int} [compareMode] 比较方式
...
//    @Permission(permissionName = "mdmbase:")
     ...
     * @apiParam (请求参数) {Int} companyID 公司ID
     * @apiParam (请求参数) {Boolean} [isEmptyCoverage] 是否空值覆盖.<br>若之前有配置过某个传感器属性的某一级配置(记为A),在本次批量配置中又配置到了该传感器属性的另一级配置(记为B,且A!=B),但A级配置处为空时,如果本项为true,则会覆盖掉之前A级的配置
     * @apiParam (请求参数) {Int[]} sensorIDList 传感器ID List
     * @apiParam (请求参数) {Object[]} dataList 数据列表
     * @apiParam (请求参数) {Int} dataList.fieldID 传感器ID
     * @apiParam (请求参数) {String} dataList.warnName 报警名称
     * @apiParam (请求参数) {Int} dataList.compareMode 比较方式
...
//    @Permission(permissionName = "mdmbase:")
     ...
     * @apiSuccess (返回结果) {Int} triggerTimes 满足规则触发报警的次数,triggerType==1时该项恒为1
     * @apiSuccess (返回结果) {Object[]} warnLevel 报警枚举
     ...
     * @apiSuccess (返回结果) {Int} fieldList.fieldID 属性ID
     * @apiSuccess (返回结果) {String} fieldList.fieldName 属性名称
     * @apiSuccess (返回结果) {String} fieldList.fieldToken 属性token
...
//    @Permission(permissionName = "mdmbase:")
     ...
     * @apiParam (请求参数) {Int} platform 平台
     * @apiParam (请求参数) {Int} monitorItemID 监测项目ID
     * @apiParam (请求参数) {Int} [triggerTimes] 满足规则触发报警的次数
     * @apiParam (请求参数) {Object[]} [aliasConfigList] 别名配置列表
     * @apiParam (请求参数) {Int} aliasConfigList.fieldID 属性ID
