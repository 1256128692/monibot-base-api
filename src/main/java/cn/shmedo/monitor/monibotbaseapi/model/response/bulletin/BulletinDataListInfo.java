 ...
 */
@Data
public class BulletinDataListInfo {
    ...
    public void handlePlatform(final Map<Integer, String> serviceIdNameMap) {
        Optional.ofNullable(platformStr).filter(StrUtil::isNotEmpty).ifPresent(u -> {
            this.setPlatform(Arrays.stream(u.split(",")).map(Integer::parseInt).toList());
            List<String> list = platform.stream().map(serviceIdNameMap::get).filter(Objects::nonNull).toList();
            this.setPlatformDesc(String.join(",", list));
        });
    }
}
